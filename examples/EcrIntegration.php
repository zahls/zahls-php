...
use Zahls\Zahls;
use Zahls\Models\Request\EcrPairing;
use Zahls\Models\Request\EcrPayment;
...
$zahls = new Zahls($instanceName, $apiSecret);

try {
    // 1. Pair Terminal
    // This connects the physical terminal to your specific Zahls instance.
    ...
    try {
        $pairResponse = $zahls->pair($ecrPair);
    ...
    $methodsResponse = $zahls->getEcrPaymentMethods($ecrMethods);
    ...
    $payResponse = $zahls->payment($ecrPay);
    ...
    if ($paymentId) {
        ...
        $checkResponse = $zahls->getEcrPayment($ecrCheck);
        ...
        try {
            $cancelResponse = $zahls->cancelEcrPayment($ecrCancel);
        ...
        try {
            $voidResponse = $zahls->voidEcrPayment($ecrVoid);
    ...
    $unpairResponse = $zahls->unpair($ecrUnpair);
